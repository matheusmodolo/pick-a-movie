<template>
    <div class="max-w-6xl mx-auto px-4 py-6">
        <h2 class="font-display text-3xl text-white mb-4">
            ...
            <span v-if="watchlist.length" class="text-lg text-gray-400">({{ watchlist.length }})</span>
        </h2>
        ...
        <div v-else-if="watchlist.length" class="grid grid-cols-1 gap-4">
            <div v-for="entry in watchlist" :key="entry.id"
                ...
                <div class="flex gap-4 flex-1">
                    ...
                    <div class="flex-1 flex flex-col justify-between">
                        <div>
                            <h3 class="text-white font-semibold">{{ entry.movie?.title || 'Título desconhecido' }}</h3>
                            <p class="text-sm text-gray-400">{{ entry.movie?.year }}</p>
                        </div>
                        <p class="text-sm text-gray-400">Adicionado em {{ entry.created_at }}</p>
                    </div>
                </div>
